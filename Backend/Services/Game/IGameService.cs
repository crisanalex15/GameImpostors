...
namespace Backend.Services.Game
{
    ...
    {
        // Lobby Management
        Task<(bool Success, GameObject? Game, string Message)> CreateGameAsync(Guid hostId, GameType gameType, int maxPlayers = 6);
        Task<(bool Success, GameObject? Game, string Message)> JoinGameAsync(string lobbyCode, Guid userId);
