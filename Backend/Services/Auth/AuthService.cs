...
namespace Backend.Services.Auth
{
    public interface IAuthService
    {
        ...
        Task<UserProfileRequestDTO?> GetMinimalProfileAsync(string userId);
    }
    ...
    public class AuthService : IAuthService
    {
        ...
        public async Task<UserProfileRequestDTO?> GetMinimalProfileAsync(string userId)
        {
            try
            {
                var user = await _userManager.FindByIdAsync(userId);
                ...
                // use dto
                var userDto = new UserProfileRequestDTO()
                {
                    Id = user.Id,
                    Email = user.Email ?? "",
                    ...
                    LastModifiedAt = user.LastModifiedAt ?? DateTime.UtcNow,
                };
                return userDto;
            }
            catch (Exception ex)
            {
                _logger.LogError(ex, "Error during getting minimal profile for user: {UserId}", userId);
                throw;
            }
        }
