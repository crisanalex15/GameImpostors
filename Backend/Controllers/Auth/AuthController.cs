...
namespace Backend.Controllers.Auth
{
    ...
    public class AuthController : ControllerBase
    {
        ...
        public async Task<IActionResult> UpdateUsername([FromBody] UpdateUsernameRequestDTO request)
        {
            ...
            {
                _logger.LogError(ex, "Error during username update");
                return StatusCode(500, new { Message = "An error occurred while updating the username." });
            }
        }

        [HttpGet("minimal-profile")]
        [Authorize(AuthenticationSchemes = JwtBearerDefaults.AuthenticationScheme)]
        public async Task<IActionResult> MinimalProfile()
        {
            var userId = User.FindFirst(ClaimTypes.NameIdentifier)?.Value;
            if (userId == null)
            {
            ...
            var userProfile = await _authService.GetMinimalProfileAsync(userId);
            if (userProfile == null)
            {
                return NotFound(new { Message = "User not found." });
            }
        ...
        [HttpGet("getMinimalProfile")]
        [Authorize(AuthenticationSchemes = JwtBearerDefaults.AuthenticationScheme)]
        public async Task<IActionResult> GetMinimalProfile(string userId)
        {
            if (userId == null)
            {
                return Unauthorized(new { Message = "User not found." });
            }
            var user = await _authService.GetMinimalProfileAsync(userId);
            return Ok(user);
        }
